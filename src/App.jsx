...
);  const AppLogo = () => (  <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>    
...
}  return (  <div style={{ background: "#f3f7ff", minHeight: "100vh", paddingBottom: "125px", fontFamily: "Arial" }}>    
<Header  
  usuario={usuario}  
  nome={nome}  
  saldo={saldo}  
...
</h2>  <p>Objetivo: <strong>{moeda(valorMetaTotal)}</strong></p>      
    <p>Guardado: <strong>{moeda(valorGuardado)}</strong></p>      
    <p>Falta: <strong>{moeda(faltaMeta)}</strong></p>      
    <p><strong>{mensagemMetaEspecial}</strong></p>      <div style={{ width: "100%", height: "20px", background: "#e5e7eb", borderRadius: "999px", overflow: "hidden", marginTop: "12px" }}>      
  <div style={{ width: progressoMeta + "%", height: "20px", background: "linear-gradient(90deg,#0D47A1,#42a5f5)", borderRadius: "999px" }} />      
</div>

  </div>        <div style={card}>      
    <h2 style={{ marginTop: 0 }}>🚦 Alertas inteligentes</h2>      {alertasInteligentes.map((item, index) => (      
...
<button onClick={adicionarGasto} style={primaryButton}>➕ Adicionar gasto +5 XP</button>  
    ...
    {gastosPorCategoria.length === 0 ? <p>Nenhum gasto lançado ainda.</p> : gastosPorCategoria.map((item) => {      
      const largura = (item.total / maiorGastoCategoria) * 100;      return (      
...
<div style={{ padding: "20px" }}>  
<div style={card}>  
<h2>📈 Histórico Financeiro</h2>  {historico.length === 0 && <p>Nenhum mês salvo ainda.</p>}
...
<div style={{ padding: "20px" }}>  
<div style={card}>  
<h2>👤 Perfil</h2>  <div style={{ textAlign: "center", marginBottom: "20px" }}>      
      {fotoAtual ? (      
